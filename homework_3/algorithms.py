...
def gd(filename, x_init, A, y, gamma, 
    ...
    for it in range(S):
        ...
        g_k = logreg_grad_plus_lasso(x, args=[A, y, l2, sparse, l1])
        x = x - gamma * g_k
...
def FISTA(filename, x_init, A, y, L, 
    ...
    for it in range(S):
        ...
        #ваш код здесь (возможно, ещё где-то придётся вставить код)
        g_k = logreg_grad(yk, args=[A, y, mu, sparse])
        xk1 = prox_R(yk - g_k / L, l1 / L)
        tk1 = (1 + np.sqrt(1 + 4 * tk ** 2)) / 2
        # yk = xk1 + tk / tk1 * (xk1 - xk)
...
def prox_gd(filename, x_init, A, y, gamma, 
    ...
    for it in range(S):
        ...
        #ваш код здесь
        g_k = logreg_grad(x, args=[A, y, l2, sparse])
...
def svrg(filename, x_init, A, y, gamma, 
    ...
    for s in range(S):
        ...
        w_grad = logreg_grad(w, [A_for_full, y, l2, sparse_full])
        for it in range(M):
